...
public func alignUp(_ value: Int, to alignment: Int) -> Int {
    precondition(alignment > 0 && (alignment & (alignment - 1)) == 0, "alignment must be a positive power of two")
    let mask = alignment - 1
    // Prevent overflow: if value > Int.max - mask, rounding would wrap.
    if value > Int.max - mask { return value }
